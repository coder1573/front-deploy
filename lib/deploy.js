...
function startZipPromise(distPath, projectDir) {
  return new Promise((resolve, reject) => {
    ...
    const output = fs.createWriteStream(pathJoin(projectDir, 'dist.zip'));
    output.on('close', err => {
      if (err) {
        reject(err);
        process.exit(1);
    ...
    if (isIncremental) { // 增量部署，部分打包
      // 添加文件
      archive.file(pathJoin(distPath, 'index.html'), { name: 'index.html' });
      archive.file(pathJoin(distPath, 'index.html.gz'), { name: 'index.html.gz' });
      // 添加目录
      archive.directory(pathJoin(distPath, 'static'), 'static');
    } else { // 全量部署，全部打包
      // 添加目录
...
async function uploadFile(webDir, projectDir) {
  try {
    ...
    await ssh.putFile(pathJoin(projectDir, 'dist.zip'), pathJoin(webDir, 'dist.zip'));
    successLog('  zip包上传成功');
  } catch (err) {
...
async function deleteWebFile(webDir) {
  try {
    ...
    if (isIncremental) {
      // 增量部署，删除部分
      await runSSHCommand(`rm -rf ${pathJoin(webDir, 'index.html')}`, { cwd: webDir });
      await runSSHCommand(`rm -rf ${pathJoin(webDir, 'index.html.gz')}`, { cwd: webDir });
      await runSSHCommand(`rm -rf ${pathJoin(webDir, 'static')}`, { cwd: webDir });
...
async function backupWebFile(webDir, backupDir) {
  try {
    ...
    // 构建压缩文件路径
    const tarFullPath = pathJoin(backupDir, `${webFolder || '未命名'}_${timeStr}.tar.gz`);
...
async function deleteLocalZip(projectDir) {
  return new Promise((resolve, reject) => {
    const zipFile = pathJoin(projectDir, 'dist.zip')
